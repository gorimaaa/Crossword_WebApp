class Grid{
	...
	loadGrid(solutionsRows, solutionsColumns, hintsRows, hintsColumns) {
	    this.#updateGrid();

	    const cells = document.getElementsByClassName('cell');
	    let cellIndex = 0;

	    // Charger les solutions dans les cellules
	    for (let i = 0; i < solutionsRows.length; i++) {
	        for (let j = 0; j < solutionsRows[i].length; j++) {
	            const cell = cells[cellIndex];
	            if (solutionsRows[i][j] === "$") {
	    ...
	    // Charger les indices des lignes
	    const rowsHintsContainer = document.getElementById('rows-hints');
	    rowsHintsContainer.innerHTML = ""; // Effacer les indices existants
	    for (let i = 0; i < hintsRows.length; i++) {
	        const hintRow = document.createElement('p');
	        hintRow.textContent = `${i + 1} - ${hintsRows[i].join(" | ")}`; // Séparation par un marqueur
	        rowsHintsContainer.appendChild(hintRow);
	    }

	    // Charger les indices des colonnes
	    const columnsHintsContainer = document.getElementById('columns-hints');
	    columnsHintsContainer.innerHTML = ""; // Effacer les indices existants
	    for (let i = 0; i < hintsColumns.length; i++) {
	        const hintColumn = document.createElement('p');
	        hintColumn.textContent = `${String.fromCharCode("A".charCodeAt(0) + i)} - ${hintsColumns[i].join(" | ")}`; // Séparation par un marqueur
	        columnsHintsContainer.appendChild(hintColumn);
	    }
	}
...
let grid; // Déclarez `grid` en tant que variable globale
function main(){
	grid = new Grid(dimX, dimY);
	grid.loadGrid(solutionsRows, solutionsColumns, hintsRows, hintsColumns);
}
